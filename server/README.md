...
```
XMEM_API_KEYS=key-one,key-two XMEM_MODERATOR_KEYS=moderator-one PORT=8787 node dist-server/main.js
```
...
- `POST /v1/reports` for publisher abuse reports; `GET` requires a key in `XMEM_MODERATOR_KEYS`
